# PARTS TRACKER: Mad Maxine

## Document Purpose

This document tracks all parts needed for the restoration, organized by phase. Use this to plan purchases, track orders, and monitor spending. Update status as parts are ordered, received, and installed.

**Budget Target:** $3,000 - $5,000  
**Current Parts Spend:** $1,750.94 (excludes tools tracked separately in TOOLS_INVENTORY.md)  
**Tools Spend:** $95.80 (tracked in TOOLS_INVENTORY.md)  
**Total Spend:** $1,846.74

## Document Relationship

This tracker organizes all parts by phase for shopping and installation tracking.

**For technical specifications and general sourcing:** See **Jeep_Comanche_Project_Brief.md**  
**For task context:** See **03_MASTER_RESTORATION_PLAN.md**

This document focuses on: What to buy, where to buy it, when to buy it, and tracking purchase/installation status.

---

## Status Codes

| Code | Meaning                       |
| ---- | ----------------------------- |
| 📋   | Needed - not yet ordered      |
| 🛒   | Ordered - awaiting delivery   |
| 📦   | Received - in inventory       |
| ✅   | Installed - complete          |
| ❌   | Not needed (after assessment) |
| ⚠️   | Backordered or delayed        |

---

## PHASE 0: PLANNING & ASSESSMENT

### Supplies & Consumables

| Item             | Qty       | Est. Cost | Source     | Part# | Status | Actual Cost | Notes                         |
| ---------------- | --------- | --------- | ---------- | ----- | ------ | ----------- | ----------------------------- |
| Engine degreaser | 1 gal     | $15       | Auto parts | -     | 📋     |             | Gunk engine cleaner           |
| UV dye for oil   | 3 bottles | $12       | AutoZone   | 374CS | 📦     | $23.97      | Auto Pro, Order #210627708473 |
| Rust converter   | 1 qt      | $25       | Auto parts | -     | 📋     |             | POR-15 or Rustoleum           |

**Phase 0 Subtotal:** $52 estimated | $23.97 actual

**Note:** Tools have been moved to TOOLS_INVENTORY.md document for separate tracking.

---

## PHASE 1: SAFETY & STRUCTURAL

### 1.1 Exhaust System

| Item                    | Qty | Est. Cost | Source     | Part# | Status | Actual Cost | Notes                          |
| ----------------------- | --- | --------- | ---------- | ----- | ------ | ----------- | ------------------------------ |
| Exhaust manifold        | 1   | $85       | RockAuto   | -     | 📋     |             | **IF CRACKED** - inspect first |
| Exhaust manifold gasket | 1   | $15       | RockAuto   | -     | 📋     |             |                                |
| Donut gasket            | 1   | $10       | Auto parts | -     | 📋     |             | Manifold to pipe               |
| Header pipe             | 1   | $80       | RockAuto   | -     | 📋     |             | Manifold to cat                |
| Catalytic converter     | 1   | $120      | RockAuto   | -     | 📋     |             |                                |
| Muffler                 | 1   | $60       | RockAuto   | -     | 📋     |             |                                |
| Tail pipe               | 1   | $40       | RockAuto   | -     | 📋     |             |                                |
| Exhaust hangers         | 4-6 | $20       | Auto parts | -     | 📋     |             |                                |
| Exhaust clamps          | 4-6 | $15       | Auto parts | -     | 📋     |             |                                |

**Exhaust Subtotal:** $360-445 (depending on manifold condition)

**Sourcing Notes:**

- Measure existing exhaust before ordering
- Walker brand is good quality for reasonable price
- Check if emissions testing required in your area (affects cat choice)

### 1.2 Driver Door Repair

| Item                      | Qty   | Est. Cost | Source         | Part# | Status | Actual Cost | Notes                               |
| ------------------------- | ----- | --------- | -------------- | ----- | ------ | ----------- | ----------------------------------- |
| Door hinges (upper/lower) | 1 set | $80       | RockAuto       | -     | 📋     |             | **IF NEEDED** after inspection      |
| Complete door assembly    | 1     | $100      | Junkyard       | -     | 📋     |             | **IF NEEDED** if door shell damaged |
| Welding rod/wire          | 5 lb  | $30       | Welding supply | -     | 📋     |             | For repairs                         |

**Door Repair Subtotal:** $30-210 (depending on damage severity)

**Note:** Welding class moved to TOOLS_INVENTORY.md as future training/equipment investment.

**Sourcing Notes:**

- Inspect first before ordering
- XJ Cherokee 2-door doors may interchange
- Consider professional welding if structural ($100-200 labor)

### 1.3 Floor Pan Repair

| Item                  | Qty       | Est. Cost | Source         | Part# | Status | Actual Cost | Notes                            |
| --------------------- | --------- | --------- | -------------- | ----- | ------ | ----------- | -------------------------------- |
| Sheet metal (16-18ga) | 2-3 sq ft | $30       | Metal supplier | -     | 📋     |             | For patches                      |
| Full floor pan kit    | 1 set     | $250      | Amazon/eBay    | -     | 📋     |             | **IF NEEDED** for extensive rust |
| Weld-through primer   | 1 can     | $15       | Auto parts     | -     | 📋     |             |                                  |
| POR-15 sealer         | 1 qt      | $40       | Amazon         | -     | 📋     |             | After welding                    |

**Floor Pan Subtotal:** $85-305 (patch vs. full replacement)

**Sourcing Notes:**

- Measure holes in Phase 0 to determine need
- Full pans available from Morris 4x4 (out of business), Quadratec (maybe), ebay
- Can fabricate patches from sheet metal if holes are small
- See ebay seller for full pans: [Classic 2 Current Fabrication](https://www.ebay.com/sch/i.html?_ssn=classic2currentfabrication&store_name=classic2currentfabrication)

### 1.4 Front Driveshaft Replacement

| Item                   | Qty | Est. Cost | Source     | Part#               | Status | Actual Cost | Notes              |
| ---------------------- | --- | --------- | ---------- | ------------------- | ------ | ----------- | ------------------ |
| Front driveshaft (new) | 1   | $100      | Parts Geek | Replacement 409-419 | 📦     | $185.16     | Ordered 2026-01-25 |

**Driveshaft Subtotal:** $150 estimated | $185.16 actual

**Sourcing Notes:**

- XJ Cherokee front driveshaft often interchanges (verify length)
- Measure collapsed length and extended length before ordering
- Check local junkyards first (cheaper than online + shipping)
- New U-joints recommended even for used shaft

### 1.5 Brake System

| Item                                        | Qty   | Est. Cost | Source               | Part#                  | Status | Actual Cost | Notes                                |
| ------------------------------------------- | ----- | --------- | -------------------- | ---------------------- | ------ | ----------- | ------------------------------------ |
| **Front brakes:**                           |       |           |                      |                        |        |             |                                      |
| Brake pads (front)                          | 1 set | $40       | RockAuto             | CENTRIC 10602030       | 📦     | $26.99      | Ordered 2026-01-30                   |
| Brake rotors (front)                        | 2     | $80       | RockAuto             | -                      | 📋     |             | **IF NEEDED** - measure first        |
| Caliper rebuild kit                         | 2     | $30       | RockAuto             | -                      | 📋     |             | **IF NEEDED**                        |
| Brake calipers (remanufactured)             | 2     | $120      | RockAuto             | -                      | 📋     |             | **IF NEEDED** if seized              |
| Front brake hoses                           | 2     | $30       | Auto parts           | -                      | 📋     |             | **IF NEEDED** if cracked             |
| **Rear brakes:**                            |       |           |                      |                        |        |             |                                      |
| Brake shoes (rear)                          | 1 set | $40       | RockAuto             | RAYBESTOS 481PG        | 📦     | $34.89      | Ordered 2026-01-30                   |
| Brake drums (rear)                          | 2     | $90       | RockAuto             | -                      | 📋     |             | **IF NEEDED** - measure first        |
| Wheel cylinder (rear left)                  | 1     | $20       | RockAuto             | RAYBESTOS WC17507      | 📦     | $7.27       | Ordered 2026-01-30                   |
| Wheel cylinder (rear right)                 | 1     | $20       | RockAuto             | RAYBESTOS WC17508      | 📦     | $7.28       | Ordered 2026-01-30                   |
| Brake hardware kit (rear)                   | 1 set | $20       | RockAuto             | RAYBESTOS H7149        | 📦     | $5.19       | Ordered 2026-01-30                   |
| Drum brake self adjuster kit (left)         | 1     | $15       | RockAuto             | RAYBESTOS H2514        | 📦     | $7.54       | Ordered 2026-01-30                   |
| Drum brake self adjuster kit (right)        | 1     | $15       | RockAuto             | RAYBESTOS H2515        | 📦     | $10.83      | Ordered 2026-01-30                   |
| **System components:**                      |       |           |                      |                        |        |             |                                      |
| Brake fluid (DOT 3/4)                       | 3 qt  | $15       | Auto parts           | $33                    | 📦     |             | For flush                            |
| Brake lubricant                             | 1     | $5        | RockAuto             | PERMATEX 85188         | 📦     | $2.02       | Ordered 2026-01-30                   |
| Master cylinder                             | 1     | $60       | RockAuto             | CENTRIC 13163031       | 📦     | $51.79      | Ordered 2026-01-30                   |
| Power brake booster check valve             | 1     | $8        | RockAuto             | DORMAN 80189           | 📦     | $3.82       | Ordered 2026-01-30                   |
| Power brake booster                         | 1     | $60       | RockAuto             | SKP SK5474040          | 📦     | $59.79      | Ordered 2026-02-17                   |
| Brake pedal pad                             | 2     | $10       | RockAuto             | FAIRCHILD D4108        | 📦     | $5.86       | Ordered 2026-01-30                   |
| Brake lines (steel, pre-bent)               | Kit   | $80       | RockAuto/Inline Tube | -                      | 📋     |             | **IF NEEDED** if rusted              |
| Brake line fitting assortment               | 1     | $20       | Auto parts           | -                      | 📋     |             | **IF NEEDED**                        |
| Brake hoses (stainless steel, front & rear) | 1 kit | $90       | Quadratec            | RC 16211-5212          | 📦     | $89.95      | Ordered 2026-02-06, 4-6" lift        |
| **Parking brake:**                          |       |           |                      |                        |        |             |                                      |
| Parking brake cable (front)                 | 1     | $30       | RockAuto             | RAYBESTOS BC93549      | 📦     | $21.79      | Ordered 2026-01-26                   |
| Parking brake cable (rear)                  | 1     | $30       | RockAuto             | RAYBESTOS BC93550      | 📦     | $24.79      | Ordered 2026-01-26                   |
| Parking brake cable (rear, re-order)        | 1     | $30       | RockAuto             | RAYBESTOS BC93550      | 🛒     | $24.79      | Ordered 2026-02-21, Order #337591052 |
| Parking brake strut spring                  | 2     | $5        | RockAuto             | RAYBESTOS H1338        | 📦     | $3.80       | Ordered 2026-01-26                   |
| Parking brake lever assembly                | 1     | $40       | Junkyard             | -                      | 📋     |             | **IF MISSING**                       |
| **Wheels & Hardware:**                      |       |           |                      |                        |        |             |                                      |
| Lug nuts                                    | 20    | $25       | RockAuto             | DORMAN 611154          | 📦     | $22.20      | Ordered 2026-01-30                   |
| Lug studs (front)                           | 10    | $10       | RockAuto             | DORMAN 610364          | 📦     | $4.70       | Missing 9 of 10                      |
| Lug studs (rear, longer)                    | 5     | $5        | RockAuto             | DORMAN 610256          | 📦     | $2.60       | Ordered 2026-01-30                   |
| Lug studs (rear, alt part #)                | 5     | $5        | RockAuto             | DORMAN 610256 (981881) | 📦     | $2.60       | Ordered 2026-01-30                   |

**Brake System Subtotal:** $200-650 estimated | $420.49 actual

**Sourcing Notes:**

- Assess condition in Phase 0 before ordering
- Wagner or Raybestos pads/shoes good quality
- Centric rotors/drums good quality
- Consider stainless steel brake lines for rust resistance (+$40)

### 1.6 Steering & Suspension System

| Item                                                | Qty   | Est. Cost | Source        | Part#          | Status | Actual Cost | Notes                                |
| --------------------------------------------------- | ----- | --------- | ------------- | -------------- | ------ | ----------- | ------------------------------------ |
| Tie rod ends (inner/outer)                          | 1 set | $80       | RockAuto      | -              | 📋     |             | Moog preferred                       |
| Drag link                                           | 1     | $60       | RockAuto      | -              | 📋     |             | **IF NEEDED**                        |
| Center link                                         | 1     | $50       | RockAuto      | -              | 📋     |             | **IF NEEDED**                        |
| Ball joints (upper)                                 | 2     | $60       | RockAuto      | -              | 📋     |             | **IF NEEDED** - test first           |
| Ball joints (lower)                                 | 2     | $60       | RockAuto      | -              | 📋     |             | **IF NEEDED** - test first           |
| Control arm bushing kit (upper)                     | 2     | $20       | RockAuto      | MOOG K3128     | 📦     | $18.24      | Ordered 2026-01-26, Qty 2 total      |
| Control arm bushing kit (lower)                     | 2     | $28       | RockAuto      | MOOG K3131     | 📦     | $26.44      | Ordered 2026-01-26, Qty 2 total      |
| Sway bar link                                       | 2     | $15       | RockAuto      | MOOG K3150     | 📦     | $13.50      | Ordered 2026-02-17, Qty 2            |
| Sway bar bushing                                    | 1     | $10       | RockAuto      | MOOG K3170     | 📦     | $4.59       | Ordered 2026-02-17                   |
| Power steering hoses                                | 2     | $40       | Auto parts    | -              | 📋     |             | **IF NEEDED** if leaking             |
| Power steering fluid                                | 1 qt  | $10       | Auto parts    | -              | 📋     |             | Top-off or flush                     |
| Crown Automotive Shock Isolators                    | 2     | $20       | Quadratec     | 56020-100      | 📦     | $16.78      | Ordered 2026-02-06                   |
| Rough Country N3 Steering Stabilizer                | 1     | $40       | Quadratec     | RC 16211-1700  | 📦     | $39.95      | Ordered 2026-02-06                   |
| Rough Country 3" Lift Kit w/ Add-A-Leaf & N3 Shocks | 1     | $350      | Rough Country | 16211-0260     | 📦     | $349.95     | Ordered 2026-02-03                   |
| Control arm (lower, driver side)                    | 1     | $25       | RockAuto      | DORMAN 520-337 | 🛒     | $21.79      | Ordered 2026-02-21, Order #337591052 |
| Control arm (lower, passenger side)                 | 1     | $25       | RockAuto      | DORMAN 520-337 | 🛒     | $21.79      | Ordered 2026-02-21, Order #337591052 |

**Steering & Suspension Subtotal:** $553-823 estimated | $513.03 actual

**Sourcing Notes:**

- Moog "Problem Solver" series preferred for quality
- Test all components in Phase 0 before ordering
- Ball joints may require press (mobile mechanic $100-150 labor)
- Complete lift kit includes new springs and N3 shocks
- Will require alignment after installation

### 1.7 Rust Prevention & Undercoating

| Item                    | Qty    | Est. Cost | Source     | Part# | Status | Actual Cost | Notes                        |
| ----------------------- | ------ | --------- | ---------- | ----- | ------ | ----------- | ---------------------------- |
| Rust converter          | 1 qt   | $25       | Auto parts | -     | 📋     |             | POR-15 Metal Prep or similar |
| POR-15 chassis paint    | 1 qt   | $60       | Amazon     | -     | 📋     |             | Frame coating                |
| POR-15 top coat (black) | 1 qt   | $45       | Amazon     | -     | 📋     |             | Over chassis paint           |
| Fluid Film (5 gal)      | 1      | $120      | Amazon     | -     | 📋     |             | Annual undercoating          |
| Cavity wax (aerosol)    | 4 cans | $40       | Auto parts | -     | 📋     |             | Inside frame rails           |

**Rust Prevention Subtotal:** $290

**Note:** Brushes/rollers moved to TOOLS_INVENTORY.md.

**Sourcing Notes:**

- POR-15 is expensive but very durable
- Fluid Film or Wool Wax for annual application (oil-based, penetrating)
- Do NOT use rubberized undercoating (traps moisture)
- Consider renting spray equipment for Fluid Film application

### 1.8 Weatherproofing

| Item                      | Qty      | Est. Cost | Source     | Part#              | Status | Actual Cost | Notes                    |
| ------------------------- | -------- | --------- | ---------- | ------------------ | ------ | ----------- | ------------------------ |
| Door seals (weatherstrip) | 1        | $120      | RockAuto   | PRECISION DK611084 | 📦     | $115.79     | Ordered 2026-02-17       |
| Cowl seal                 | 1        | $10       | RockAuto   | PRECISION CS611084 | 📦     | $8.89       | Ordered 2026-02-17       |
| Windshield gasket         | 1        | $40       | RockAuto   | -                  | 📋     |             | **IF NEEDED** if leaking |
| Rear window seal          | 1        | $25       | RockAuto   | -                  | 📋     |             | **IF NEEDED**            |
| Seam sealer               | 2 tubes  | $20       | Auto parts | -                  | 📋     |             | Body seam sealing        |
| Firewall grommets         | Assorted | $15       | Auto parts | -                  | 📋     |             | Wire/hose penetrations   |

**Weatherproofing Subtotal:** $100-150 estimated | $124.68 actual

### 1.9 Hood & Exterior

| Item                   | Qty | Est. Cost | Source   | Part#             | Status | Actual Cost | Notes                            |
| ---------------------- | --- | --------- | -------- | ----------------- | ------ | ----------- | -------------------------------- |
| Hood release cable     | 1   | $25       | RockAuto | DORMAN 912008     | 📦     | $24.79      | Ordered 2026-01-26, fits 1992 MJ |
| Headlamp bezel (left)  | 1   | $12       | RockAuto | TECHPRO CH2512108 | 📦     | $10.02      | Ordered 2026-02-17               |
| Headlamp bezel (right) | 1   | $12       | RockAuto | TECHPRO CH2513108 | 📦     | $10.02      | Ordered 2026-02-17               |

**Hood & Exterior Subtotal:** $25 estimated | $44.83 actual

---

**PHASE 1 TOTAL:** $1,608-2,185 estimated | $1,288.19 actual

---

## PHASE 2: DRIVETRAIN RELIABILITY

### 2.1 Engine Oil Leak Repairs

| Item                          | Qty    | Est. Cost | Source     | Part#                          | Status | Actual Cost | Notes                     |
| ----------------------------- | ------ | --------- | ---------- | ------------------------------ | ------ | ----------- | ------------------------- |
| Oil pressure sending unit     | 1      | $25       | RockAuto   | STANDARD MOTOR PRODUCTS PS155T | 📦     | $11.35      | Ordered 2026-01-30        |
| Valve cover gasket            | 2      | $15       | RockAuto   | FEL-PRO VS50245R               | 📦     | $4.00       | Ordered 2026-01-30, Qty 2 |
| Oil filter adapter O-ring     | 1      | $5        | RockAuto   | FEL-PRO 70301                  | 📦     | $0.65       | Ordered 2026-01-30        |
| Oil Filter O-ring             | 1      | $10       | RockAuto   | MAHLE/CLEVITE 72219            | 🛒     | $0.86       | Ordered 2026-01-30        |
| Oil filter O-ring             | 1      | $5        | RockAuto   | MAHLE/CLEVITE 72217            | 📦     | $0.50       | Ordered 2026-01-30        |
| RTV silicone gasket maker     | 1 tube | $10       | Auto parts | -                              | 📋     |             | Per FSM for valve cover   |
| Rear main seal                | 1      | $20       | Auto parts | -                              | 📋     |             | **IF NEEDED**             |
| Oil pan gasket                | 1      | $20       | Auto parts | -                              | 📋     |             | **IF NEEDED**             |
| Front crankshaft seal         | 1      | $15       | Auto parts | -                              | 📋     |             | **IF NEEDED**             |
| Engine oil (10W-30 or 10W-40) | 5 qt   | $25       | Auto parts | -                              | 📋     |             | For changes during repair |
| Oil filter                    | 2      | $12       | Auto parts | -                              | 📋     |             | Wix 51515 or equivalent   |

**Oil Leak Repairs Subtotal:** $55-152 estimated | $17.36 actual

### 2.2 Ignition System

| Item                             | Qty | Est. Cost | Source     | Part#                          | Status | Actual Cost | Notes                                |
| -------------------------------- | --- | --------- | ---------- | ------------------------------ | ------ | ----------- | ------------------------------------ |
| Ignition starter switch          | 1   | $50       | RockAuto   | SKP SKUS105                    | 📦     | $6.01       | Ordered 2026-01-24                   |
| Ignition lock cylinder           | 1   | $10       | RockAuto   | WELLS 4H1019                   | 📦     | $7.59       | Ordered 2026-01-24                   |
| Starter motor control relay      | 1   | $20       | RockAuto   | STANDARD MOTOR PRODUCTS SR105T | 🛒     | $15.00      | Ordered 2026-02-21, Order #337591052 |
| Steering column repair connector | 1   | $15       | Auto parts | -                              | 📋     |             | **IF NEEDED** if melted              |

**Ignition System Subtotal:** $60-75 estimated | $28.60 actual

### 2.3 Engine Tune-Up

| Item                    | Qty   | Est. Cost | Source     | Part#                        | Status | Actual Cost | Notes                                                  |
| ----------------------- | ----- | --------- | ---------- | ---------------------------- | ------ | ----------- | ------------------------------------------------------ |
| Spark plugs             | 4     | $15       | RockAuto   | NGK 7252                     | 📦     | $8.28       | Ordered 2026-01-30                                     |
| Spark plug wires        | 1 set | $35       | RockAuto   | NGK 53292                    | 📦     | $15.15      | Ordered 2026-01-30                                     |
| Distributor cap & rotor | 1     | $20       | RockAuto   | WELLS 3D1016A                | 📦     | $10.26      | Ordered 2026-01-30                                     |
| Throttle body gasket    | 1     | $5        | RockAuto   | FEL-PRO 60747                | 📦     | $1.39       | Ordered 2026-01-30                                     |
| Air filter              | 1     | $20       | Auto parts | -                            | 📋     |             |                                                        |
| Fuel filter             | 1     | $15       | RockAuto   | WIX 33486                    | 📦     | $12.44      | Ordered 2026-01-30                                     |
| PCV valve               | 1     | $8        | RockAuto   | STANDARD MOTOR PRODUCTS V207 | 📦     | $3.57       | Ordered 2026-01-30                                     |
| PCV grommet             | 2     | $5        | RockAuto   | DORMAN 42059                 | 📦     | $3.87       | Qty 1 @ $2.38 (2026-01-30), Qty 1 @ $1.49 (2026-02-17) |

**Tune-Up Subtotal:** $88-123 estimated | $54.96 actual

### 2.4 Cooling System

| Item                      | Qty   | Est. Cost | Source     | Part#          | Status | Actual Cost | Notes                              |
| ------------------------- | ----- | --------- | ---------- | -------------- | ------ | ----------- | ---------------------------------- |
| Thermostat (195°F)        | 1     | $15       | RockAuto   | MOTORAD 200195 | 📦     | $3.72       | Ordered 2026-01-30                 |
| Thermostat housing gasket | 1     | $5        | RockAuto   | FEL-PRO 35024  | 📦     | $0.86       | Ordered 2026-01-30                 |
| Antifreeze (concentrate)  | 3 gal | $45       | Home Depot | 1000045487     | 📦     | $40.44      | Prestone 10yr/300k mi              |
| Radiator cap              | 1     | $15       | Auto parts | -              | 📋     |             | **IF NOT HOLDING PRESSURE**        |
| Upper radiator hose       | 1     | $20       | Auto parts | -              | 📋     |             | **IF NEEDED**                      |
| Lower radiator hose       | 1     | $20       | Auto parts | -              | 📋     |             | **IF NEEDED**                      |
| Heater hoses              | 2     | $20       | Auto parts | -              | 📋     |             | **IF NEEDED**                      |
| Radiator (2-row)          | 1     | $180      | RockAuto   | -              | 📋     |             | **DEFER TO SPRING** if not leaking |
| Water pump                | 1     | $85       | RockAuto   | -              | 📋     |             | **DEFER TO SPRING** if not leaking |

**Cooling System Subtotal:** $65-405 estimated | $45.02 actual

### 2.5 Renix Sensors

| Item                             | Qty   | Est. Cost | Source     | Part#                        | Status | Actual Cost | Notes                                                    |
| -------------------------------- | ----- | --------- | ---------- | ---------------------------- | ------ | ----------- | -------------------------------------------------------- |
| Coolant temperature sensor (CTS) | 1     | $30       | RockAuto   | STANDARD MOTOR PRODUCTS TX28 | 📦     | $28.89      | Ordered 2026-01-30                                       |
| Crankshaft position sensor (CPS) | 1     | $20       | RockAuto   | WALKER PRODUCTS 235-1096     | 🛒     | $15.72      | Ordered 2026-02-21, after $1.75 rebate, Order #337591052 |
| Manifold pressure (MAP) sensor   | 1     | $35       | RockAuto   | WALKER PRODUCTS 225-91001    | 🛒     | $29.51      | Ordered 2026-02-21, after $3.28 rebate, Order #337591052 |
| Throttle position sensor (TPS)   | 1     | $35       | Auto parts | -                            | 📋     |             | **IF NEEDED** - test first                               |
| Oxygen sensor                    | 1     | $40       | Auto parts | -                            | 📋     |             | **IF NEEDED** - test first                               |
| Throttle body cleaner            | 1 can | $10       | Auto parts | -                            | 📋     |             | For IAC and TB cleaning                                  |
| Electrical contact cleaner       | 1 can | $8        | Auto parts | -                            | 📋     |             | For grounds and connectors                               |

**Sensor Service Subtotal:** $18-118 estimated | $74.12 actual

### 2.6 Battery & Charging

| Item                 | Qty | Est. Cost | Source     | Part#         | Status | Actual Cost | Notes                      |
| -------------------- | --- | --------- | ---------- | ------------- | ------ | ----------- | -------------------------- |
| Battery (Group 34)   | 1   | $150      | Auto parts | -             | 📋     |             | **IF LOAD TEST FAILS**     |
| Battery terminal kit | 1   | $12       | Auto parts | -             | 📋     |             | Cleaner and anti-corrosion |
| Battery hold-down    | 1   | $15       | Auto parts | -             | 📋     |             | **IF MISSING**             |
| Serpentine/V-belt    | 1   | $20       | RockAuto   | GATES 7430    | 📦     | $12.17      | Ordered 2026-01-30         |
| Belt                 | 1   | $20       | RockAuto   | GATES K060820 | 📦     | $20.79      | Ordered 2026-02-17         |

**Battery & Charging Subtotal:** $12-197 estimated | $32.96 actual

### 2.7 Transmission Service

| Item                           | Qty  | Est. Cost | Source     | Part# | Status | Actual Cost | Notes                  |
| ------------------------------ | ---- | --------- | ---------- | ----- | ------ | ----------- | ---------------------- |
| ATF Dexron III                 | 4 qt | $25       | Auto parts | -     | 📦     | $20.24      | AX5 transmission fluid |
| Transmission drain plug gasket | 1    | $5        | Auto parts | -     | 📋     |             | Copper crush washer    |

**Transmission Service Subtotal:** $30

### 2.8 Transfer Case Service

| Item                              | Qty    | Est. Cost | Source     | Part#         | Status | Actual Cost | Notes              |
| --------------------------------- | ------ | --------- | ---------- | ------------- | ------ | ----------- | ------------------ |
| Transfer case output seal (rear)  | 1      | $8        | RockAuto   | TIMKEN 4370N  | 📦     | $6.01       | Ordered 2026-01-24 |
| Transfer case output seal (front) | 1      | $12       | RockAuto   | TIMKEN 710046 | 📦     | $11.94      | Ordered 2026-01-24 |
| Transfer case input shaft seal    | 1      | $8        | RockAuto   | SKF 19255     | 📦     | $5.93       | Ordered 2026-01-26 |
| Transfer case gasket              | 1      | $25       | RockAuto   | -             | 📋     |             | **IF CASE LEAK**   |
| ATF Dexron III                    | 4 qt   | $20       | Auto parts | -             | 📦     | $20.24      | NP207 fluid        |
| RTV gasket maker                  | 1 tube | $10       | Auto parts | -             | 📦     |             | For sealing        |

**Transfer Case Service Subtotal:** $63-83 estimated | $23.88 actual

### 2.9 Differential Service

| Item                              | Qty    | Est. Cost | Source     | Part#            | Status | Actual Cost | Notes                           |
| --------------------------------- | ------ | --------- | ---------- | ---------------- | ------ | ----------- | ------------------------------- |
| Differential pinion seal (rear)   | 1      | $8        | RockAuto   | DANA 43085       | 📦     | $5.13       | Ordered 2026-01-24              |
| Differential pinion seal (front)  | 1      | $16       | RockAuto   | TIMKEN 5778      | 📦     | $15.91      | Ordered 2026-01-24              |
| Gear oil 80W-90 GL-5              | 4 qt   | $40       | Auto parts | -                | 🛒     |             | Front and rear diffs            |
| Diff cover gasket (front Dana 30) | 2      | $10       | RockAuto   | FEL-PRO RDS55019 | 📦     | $8.10       | Ordered 2026-01-26, Qty 2 total |
| Diff cover gasket (rear Dana 35)  | 2      | $10       | RockAuto   | FEL-PRO RDS13089 | 📦     | $7.86       | Ordered 2026-01-26, Qty 2 total |
| RTV gasket maker                  | 1 tube | $10       | Auto parts | -                | 📦     |             | For extra sealing               |

**Differential Service Subtotal:** $94 estimated | $37.00 actual

### 2.10 U-Joints

| Item                      | Qty | Est. Cost | Source     | Part#                       | Status | Actual Cost | Notes                                          |
| ------------------------- | --- | --------- | ---------- | --------------------------- | ------ | ----------- | ---------------------------------------------- |
| Rear driveshaft U-joints  | 2   | $50       | RockAuto   | DANA 5153X                  | 📦     | $26.76      | Ordered 2026-01-24, Qty 2 total                |
| Front driveshaft U-joints | 2   | $50       | Auto parts | Spicer 5-260X or equivalent | 📋     |             | **IF NEEDED** - may be included with new shaft |

**U-Joint Subtotal:** $50-100 estimated | $26.76 actual

### 2.11 Miscellaneous Fluids & Supplies

| Item                                | Qty   | Est. Cost | Source     | Part#         | Status | Actual Cost | Notes                        |
| ----------------------------------- | ----- | --------- | ---------- | ------------- | ------ | ----------- | ---------------------------- |
| Degreaser                           | 1 gal | $15       | Auto parts | -             | 📋     |             | For cleaning                 |
| Shop rags                           | 1 box | $20       | Auto parts | -             | 📋     |             |                              |
| Drain container (16 qt)             | 1     | $15       | Home Depot | 1001862948    | 📦     | $13.98      | FloTool Super Duty           |
| Funnel set                          | 1     | $10       | Auto parts | -             | 📋     |             | **IF NEEDED**                |
| RTV Ultra Black gasket maker        | 1     | $10       | Home Depot | 1002752778    | 📦     | $8.48       | Permatex, max oil resistance |
| RTV High-Temp Red gasket maker      | 1     | $10       | Home Depot | 1010441793    | 📦     | $7.98       | Permatex                     |
| Threadlocker Orange (high strength) | 1     | $12       | Home Depot | 1006099406    | 📦     | $10.98      | Permatex, removable          |
| Threadlocker Blue (medium strength) | 1     | $8        | Home Depot | 1002752779    | 📦     | $6.48       | Permatex gel                 |
| Windshield washer fluid (-20F)      | 1 gal | $5        | Home Depot | 1002339399    | 📦     | $3.68       | All season                   |
| Hose clamps (< 1")                  | 10    | $10       | RockAuto   | DORMAN 590024 | 📦     | $8.60       | Ordered 2026-02-17           |
| Vacuum hose (3/16")                 | 6 ft  | $5        | RockAuto   | GATES 27039   | 📦     | $4.68       | Ordered 2026-02-17, Qty 6    |
| Vacuum hose (1/8")                  | 6 ft  | $4        | RockAuto   | GATES 27037   | 📦     | $3.36       | Ordered 2026-02-17, Qty 6    |

**Miscellaneous Subtotal:** $65-105 estimated | $68.22 actual

### 2.12 Engine & Transmission Mounts

| Item                | Qty | Est. Cost | Source   | Part#       | Status | Actual Cost | Notes              |
| ------------------- | --- | --------- | -------- | ----------- | ------ | ----------- | ------------------ |
| Transmission mount  | 1   | $15       | RockAuto | ANCHOR 2858 | 📦     | $14.44      | Received 2026-02-2 |
| Motor mount (left)  | 1   | $10       | RockAuto | ANCHOR 2569 | 📦     | $7.66       | Received 2026-02-2 |
| Motor mount (right) | 1   | $10       | RockAuto | ANCHOR 2571 | 📦     | $7.83       | Received 2026-02-2 |

**Engine & Transmission Mounts Subtotal:** $35 estimated | $29.93 actual

### 2.13 Clutch System

| Item                   | Qty | Est. Cost | Source   | Part#      | Status | Actual Cost | Notes              |
| ---------------------- | --- | --------- | -------- | ---------- | ------ | ----------- | ------------------ |
| Clutch master cylinder | 1   | $20       | RockAuto | LUK LMC203 | 📦     | $16.32      | Ordered 2026-01-30 |
| Clutch slave cylinder  | 1   | $15       | RockAuto | LUK LSC174 | 📦     | $10.24      | Ordered 2026-01-30 |

**Clutch System Subtotal:** $35 estimated | $26.56 actual

---

**PHASE 2 TOTAL:** $518-1,420 estimated | $455.56 actual

---

## PHASE 3: ELECTRICAL SYSTEMS

### 3.1 Wiring Harness

| Item                                  | Qty     | Est. Cost    | Source             | Part# | Status | Actual Cost | Notes               |
| ------------------------------------- | ------- | ------------ | ------------------ | ----- | ------ | ----------- | ------------------- |
| **Option A: Repair existing harness** |         |              |                    |       |        |             |                     |
| Electrical connectors (assorted)      | 1 kit   | $40          | Auto parts         | -     | 📋     |             | Weatherpack, etc.   |
| Solder (rosin core)                   | 1 roll  | $15          | Electronics supply | -     | 📋     |             |                     |
| Heat shrink tubing (assorted)         | 1 kit   | $20          | Amazon             | -     | 📋     |             | Multiple sizes      |
| Wire loom (split)                     | 25 ft   | $25          | Auto parts         | -     | 📋     |             | For re-wrapping     |
| Electrical tape                       | 3 rolls | $10          | Auto parts         | -     | 📋     |             |                     |
| Dielectric grease                     | 1 tube  | $10          | Auto parts         | -     | 📋     |             | For connectors      |
| **Subtotal - Repair:**                |         | **$120**     |                    |       |        |             |                     |
| **Option B: Replace harness**         |         |              |                    |       |        |             |                     |
| Painless Performance harness          | 1       | $650         | Summit/Jegs        | -     | 📋     |             | Complete wiring kit |
| **OR** Centech harness                | 1       | $450         | Amazon/eBay        | -     | 📋     |             | Budget alternative  |
| **Subtotal - Replace:**               |         | **$450-650** |                    |       |        |             |                     |

**Wiring Harness Subtotal:** $120-650 (DECISION MADE IN PHASE 0)

### 3.2 Lighting System

| Item                                | Qty       | Est. Cost | Source     | Part# | Status | Actual Cost | Notes           |
| ----------------------------------- | --------- | --------- | ---------- | ----- | ------ | ----------- | --------------- |
| Headlight bulbs (H4 or sealed beam) | 2         | $20       | Auto parts | -     | 📋     |             |                 |
| Parking light bulbs                 | 2         | $5        | Auto parts | -     | 📋     |             |                 |
| Turn signal bulbs (front)           | 2         | $5        | Auto parts | -     | 📋     |             |                 |
| Turn signal bulbs (rear)            | 2         | $5        | Auto parts | -     | 📋     |             |                 |
| Brake light bulbs                   | 2         | $5        | Auto parts | -     | 📋     |             |                 |
| Reverse light bulbs                 | 2         | $5        | Auto parts | -     | 📋     |             |                 |
| License plate bulb                  | 1         | $3        | Auto parts | -     | 📋     |             |                 |
| Side marker bulbs                   | 4         | $8        | Auto parts | -     | 📋     |             |                 |
| Light sockets (various)             | As needed | $20       | Auto parts | -     | 📋     |             | **IF CORRODED** |
| Headlight switch                    | 1         | $30       | RockAuto   | -     | 📋     |             | **IF FAILED**   |

**Lighting System Subtotal:** $56-106

### 3.3 Gauge Cluster

| Item                             | Qty | Est. Cost | Source     | Part# | Status | Actual Cost | Notes                  |
| -------------------------------- | --- | --------- | ---------- | ----- | ------ | ----------- | ---------------------- |
| **Option A: Repair existing**    |     |           |            |       |        |             |                        |
| Speedometer cable                | 1   | $30       | RockAuto   | -     | 📋     |             | **IF MECHANICAL**      |
| Voltage regulator (cluster)      | 1   | $20       | Auto parts | -     | 📋     |             | **IF GAUGES ERRATIC**  |
| **Subtotal - Repair:**           |     | **$50**   |            |       |        |             |                        |
| **Option B: Junkyard cluster**   |     |           |            |       |        |             |                        |
| Used gauge cluster (MJ or XJ)    | 1   | $75       | Junkyard   | -     | 📋     |             | Test before buying     |
| **Subtotal - Junkyard:**         |     | **$75**   |            |       |        |             |                        |
| **Option C: Aftermarket gauges** |     |           |            |       |        |             |                        |
| Speedometer (GPS)                | 1   | $80       | Amazon     | -     | 📋     |             | Universal              |
| Fuel gauge                       | 1   | $40       | Amazon     | -     | 📋     |             | Match sending unit     |
| Temperature gauge                | 1   | $40       | Amazon     | -     | 📋     |             | Mechanical or electric |
| Gauge panel (fabricated)         | 1   | DIY       | -          | -     | 📋     |             | Sheet metal            |
| **Subtotal - Aftermarket:**      |     | **$160+** |            |       |        |             |                        |

**Gauge Cluster Subtotal:** $50-160 (DECISION BASED ON CONDITION)

### 3.4 Switches & Controls

| Item                  | Qty   | Est. Cost | Source     | Part# | Status | Actual Cost | Notes                  |
| --------------------- | ----- | --------- | ---------- | ----- | ------ | ----------- | ---------------------- |
| Ignition switch       | 1     | $50       | RockAuto   | -     | 📋     |             | **ALREADY IN PHASE 2** |
| Headlight switch      | 1     | $30       | RockAuto   | -     | 📋     |             | **IF FAILED**          |
| Turn signal switch    | 1     | $40       | RockAuto   | -     | 📋     |             | **IF FAILED**          |
| Wiper switch          | 1     | $30       | RockAuto   | -     | 📋     |             | **IF FAILED**          |
| Hazard switch         | 1     | $20       | Auto parts | -     | 📋     |             | **IF FAILED**          |
| HVAC control switches | 1 set | $30       | Junkyard   | -     | 📋     |             | **IF FAILED**          |

**Switches Subtotal:** $50-150

### 3.5 Accessories

| Item                  | Qty | Est. Cost | Source     | Part# | Status | Actual Cost | Notes                    |
| --------------------- | --- | --------- | ---------- | ----- | ------ | ----------- | ------------------------ |
| Horn                  | 1   | $20       | Auto parts | -     | 📋     |             | **IF FAILED**            |
| Wiper motor           | 1   | $50       | RockAuto   | -     | 📋     |             | **IF FAILED**            |
| Washer pump           | 1   | $20       | Auto parts | -     | 📋     |             | **IF FAILED**            |
| Wiper linkage         | 1   | $40       | Junkyard   | -     | 📋     |             | **IF BROKEN**            |
| Blower motor resistor | 1   | $25       | Auto parts | -     | 📋     |             | **IF SPEEDS DON'T WORK** |

**Accessories Subtotal:** $0-155

---

**PHASE 3 TOTAL:** $276-1,221 (wide range due to harness decision)

---

## PHASE 4: CAB & INTERIOR

### 4.1 Window Repair

| Item                      | Qty   | Est. Cost | Source     | Part# | Status | Actual Cost | Notes          |
| ------------------------- | ----- | --------- | ---------- | ----- | ------ | ----------- | -------------- |
| Window regulator (manual) | 1     | $60       | RockAuto   | -     | 📋     |             | Driver side    |
| Window crank handle       | 1     | $15       | Auto parts | -     | 📋     |             | **IF MISSING** |
| Window felt/weatherstrip  | 1 set | $25       | RockAuto   | -     | 📋     |             | **IF WORN**    |

**Window Repair Subtotal:** $60-100

### 4.2 Dash & Panel Fabrication

| Item                  | Qty       | Est. Cost | Source         | Part# | Status | Actual Cost | Notes              |
| --------------------- | --------- | --------- | -------------- | ----- | ------ | ----------- | ------------------ |
| Sheet metal (16-18ga) | 4x8 sheet | $40       | Metal supplier | -     | 📋     |             | Multiple panels    |
| Rivets/fasteners      | Assorted  | $20       | Hardware store | -     | 📋     |             | For mounting       |
| Primer (self-etching) | 1 can     | $15       | Auto parts     | -     | 📋     |             | For metal prep     |
| Paint (black)         | 2 cans    | $20       | Auto parts     | -     | 📋     |             | Rattle can or HVLP |
| Mounting hardware     | Assorted  | $15       | Hardware store | -     | 📋     |             | Screws, clips      |

**Dash Fabrication Subtotal:** $110

### 4.3 Interior Panels

| Item                   | Qty      | Est. Cost | Source         | Part# | Status | Actual Cost | Notes                    |
| ---------------------- | -------- | --------- | -------------- | ----- | ------ | ----------- | ------------------------ |
| Sheet metal or plywood | 2 sheets | $40       | Hardware store | -     | 📋     |             | Door panels, kick panels |
| Fasteners/clips        | Assorted | $20       | Hardware store | -     | 📋     |             |                          |
| Paint (black)          | 2 cans   | $20       | Auto parts     | -     | 📋     |             |                          |

**Interior Panels Subtotal:** $80

### 4.4 Floor Protection

| Item                         | Qty      | Est. Cost | Source     | Part# | Status | Actual Cost | Notes                  |
| ---------------------------- | -------- | --------- | ---------- | ----- | ------ | ----------- | ---------------------- |
| Bed liner (spray or roll-on) | 1 gallon | $50       | Auto parts | -     | 📋     |             | For floor pans         |
| Rubber floor mats            | 2        | $40       | Amazon     | -     | 📋     |             | All-weather heavy-duty |
| Floor drain plugs            | 4        | $10       | Auto parts | -     | 📋     |             | **IF MISSING**         |

**Floor Protection Subtotal:** $50-100

### 4.5 Weatherproofing

| Item                    | Qty     | Est. Cost | Source     | Part# | Status | Actual Cost | Notes                 |
| ----------------------- | ------- | --------- | ---------- | ----- | ------ | ----------- | --------------------- |
| Door seals (additional) | 2 sets  | $50       | RockAuto   | -     | 📋     |             | **IF NOT IN PHASE 1** |
| Seam sealer             | 2 tubes | $20       | Auto parts | -     | 📋     |             | Additional if needed  |

**Weatherproofing Subtotal:** $20-70

---

**PHASE 4 TOTAL:** $320-460

---

## PHASE 5: FINAL PREP & TESTING

### 5.1 Cosmetic Finish

| Item                     | Qty    | Est. Cost | Source         | Part# | Status | Actual Cost | Notes              |
| ------------------------ | ------ | --------- | -------------- | ----- | ------ | ----------- | ------------------ |
| Automotive paint (black) | 2 gal  | $120      | Paint supply   | -     | 📋     |             | For HVLP sprayer   |
| Paint thinner/reducer    | 1 gal  | $25       | Paint supply   | -     | 📋     |             | For HVLP           |
| Masking paper/tape       | 1 roll | $20       | Hardware store | -     | 📋     |             | Windows, trim      |
| Sandpaper (assorted)     | 1 pack | $15       | Hardware store | -     | 📋     |             | Light prep         |
| Tack cloth               | 1 box  | $10       | Auto parts     | -     | 📋     |             | Pre-paint cleaning |

**Cosmetic Finish Subtotal:** $190

### 5.2 Fluids & Consumables (Final Top-Off)

| Item                  | Qty      | Est. Cost | Source     | Part# | Status | Actual Cost | Notes             |
| --------------------- | -------- | --------- | ---------- | ----- | ------ | ----------- | ----------------- |
| Engine oil            | 5 qt     | $25       | Auto parts | -     | 📋     |             | Top-off or change |
| Oil filter            | 1        | $8        | Auto parts | -     | 📋     |             |                   |
| Coolant               | 1 gal    | $15       | Auto parts | -     | 📋     |             | Top-off           |
| Washer fluid (winter) | 1 gal    | $5        | Auto parts | -     | 📋     |             | -20°F rated       |
| Fuel system cleaner   | 1 bottle | $10       | Auto parts | -     | 📋     |             | Pre-test drive    |

**Fluids & Consumables Subtotal:** $63

### 5.3 Wiper Blades & Misc

| Item                    | Qty      | Est. Cost | Source     | Part# | Status | Actual Cost | Notes             |
| ----------------------- | -------- | --------- | ---------- | ----- | ------ | ----------- | ----------------- |
| Windshield wiper blades | 2        | $30       | Auto parts | -     | 📋     |             | Winter/all-season |
| Touch-up paint          | 1 bottle | $10       | Auto parts | -     | 📋     |             | Black             |
| Tire pressure gauge     | 1        | $15       | Auto parts | -     | 📋     |             | **IF NEEDED**     |

**Wiper Blades & Misc Subtotal:** $40-55

---

**PHASE 5 TOTAL:** $293-308

---

## BUDGET SUMMARY BY PHASE

| Phase       | Description            | Min Estimate | Max Estimate | Actual Spend  | Variance     |
| ----------- | ---------------------- | ------------ | ------------ | ------------- | ------------ |
| **Phase 0** | Planning & Assessment  | $52          | $52          | $23.97        | -$28.03      |
| **Phase 1** | Safety & Structural    | $1,608       | $2,185       | $1,288.19     | Savings      |
| **Phase 2** | Drivetrain Reliability | $518         | $1,420       | $455.56       | Savings      |
| **Phase 3** | Electrical Systems     | $276         | $1,221       | $0.00         | TBD          |
| **Phase 4** | Cab & Interior         | $320         | $460         | $0.00         | TBD          |
| **Phase 5** | Final Prep & Testing   | $293         | $308         | $0.00         | TBD          |
| **TOTAL**   |                        | **$3,067**   | **$5,646**   | **$1,750.94** | **On Track** |

**Target Budget:** $3,000 - $5,000  
**Contingency (20%):** $600 - $1,000  
**Total with Contingency:** $3,600 - $6,000  
**Remaining Budget:** $1,249.06 - $3,249.06

**Note:** Tool purchases ($95.80) tracked separately in TOOLS_INVENTORY.md and not included in parts budget.

---

## SOURCING QUICK REFERENCE

### Primary Parts Sources

**RockAuto.com**

- Pros: Huge selection, competitive prices, multiple quality tiers
- Cons: Shipping can be expensive, multiple warehouses = multiple shipments
- Best for: Brake parts, suspension, steering, engine parts, electrical
- Tip: Use catalog to get exact part numbers, then compare prices elsewhere

**NAPA Auto Parts**

- Pros: Local availability, easy returns, knowledgeable staff
- Cons: Higher prices than online
- Best for: Emergency parts, fluids, filters, belts, hoses
- Tip: Sign up for commercial account for discounts

**O'Reilly / AutoZone / Advance Auto**

- Pros: Local, convenient, tool loan program
- Cons: Variable prices, limited specialty parts
- Best for: Same-day needs, fluids, batteries, common wear items
- Tip: Check online for coupons and price matching

**ComancheClub.com**

- Pros: MJ-specific knowledge, used parts marketplace
- Cons: Individual sellers, condition varies
- Best for: MJ-specific parts, hard-to-find items, technical advice
- Tip: Search forums before buying new - someone may have used parts

**Local Junkyards / Pull-A-Part**

- Pros: Cheap, can inspect before buying, XJ parts interchange
- Cons: Time-consuming, condition unknown until tested
- Best for: XJ interchange parts, interior pieces, body panels, complete assemblies
- Tip: Bring tools, take photos/measurements, test electrical parts before leaving

**Amazon**

- Pros: Fast shipping (Prime), easy returns, reviews
- Cons: Quality varies wildly by seller, some knockoffs
- Best for: Tools, supplies, generic parts, aftermarket accessories
- Tip: Check seller ratings carefully, compare part numbers

**Facebook Marketplace / Craigslist**

- Pros: Local pickup, no shipping, negotiable
- Cons: Must inspect carefully, scams possible
- Best for: Complete assemblies (transmission, transfer case, axles), larger items
- Tip: Meet in public, bring cash, inspect before paying

**Quadratec / Morris 4x4 (out of business) / 4 Wheel Parts**

- Pros: Jeep specialists, quality parts, good tech support
- Cons: Higher prices
- Best for: Lift components, off-road upgrades, hard-to-find MJ parts
- Tip: Watch for sales, free shipping promotions

---

## PART NUMBER CROSS-REFERENCE

### Key Interchange Information

**XJ Cherokee Parts (1984-1996) that fit MJ Comanche:**

- Doors (2-door XJ = MJ doors)
- Window regulators
- Interior trim pieces
- Gauge clusters
- Switches and electrical
- Seats
- Engine parts (same 2.5L and 4.0L engines)
- Transmission (AX5)
- Transfer case (NP207)
- Front axle (Dana 30)
- Suspension components (similar/same)

**MJ-Specific (harder to find):**

- Bed and tailgate
- Rear quarter panels
- Some body trim

**Generic Parts:**

- Filters, fluids, belts = use standard catalogs
- Brakes, steering, suspension = match to year/model

---

## SHOPPING STRATEGY

### Bulk Ordering

Order as much as possible in single shipments to save on shipping:

1. Create Phase 1 shopping list after Phase 0 assessment
2. Order all Phase 1 parts at once from RockAuto
3. Order fluids/consumables locally
4. Order Phase 2 parts when Phase 1 is half complete

### When to Buy New vs. Used

**ALWAYS NEW:**

- Brake pads/shoes, rotors/drums
- Brake hydraulics (wheel cylinders, calipers, master cylinder)
- Steering components (tie rods, ball joints)
- Gaskets and seals
- Filters
- Fluids
- Spark plugs, ignition components

**ACCEPTABLE USED:**

- Gauge clusters (test before buying)
- Switches (test before buying)
- Interior trim pieces
- Doors and glass
- Complete assemblies (transmission, transfer case) if low miles and tested
- Driveshafts (inspect carefully for cracks/damage)

**JUNKYARD BEST:**

- Window regulators
- Interior panels
- Trim pieces
- Mirrors
- Light housings
- HVAC components

---

## PARTS ARRIVAL TRACKING

Use this section to track orders and arrivals:

| Order Date | Vendor     | Items                                                                          | Order #      | Est. Arrival | Status | Received Date |
| ---------- | ---------- | ------------------------------------------------------------------------------ | ------------ | ------------ | ------ | ------------- |
| 2026-01-15 | Home Depot | 6 items (coolant, RTV, threadlockers, washer fluid)                            | WH20875375   | 2026-01-15   | 📦     | 2026-01-15    |
| 2026-01-21 | Home Depot | Jack stands (tool), antifreeze (2), drain container                            | WH21384422   | 2026-01-27   | 📦     | 2026-01-30    |
| 2026-01-24 | RockAuto   | 7 parts (U-joints, seals, ignition)                                            | 335187809    | TBD          | 🛒     | **\_**        |
| 2026-01-24 | AutoZone   | UV dye (3), UV flashlight (tool)                                               | 210627708473 | 2026-01-26   | 📦     | 2026-01-28    |
| 2026-01-24 | Parts Geek | Front driveshaft                                                               | 31-883336    | TBD          | 📦     | 2026-01-29    |
| 2026-01-26 | RockAuto   | 12 parts (mounts, seals, bushings, cables)                                     | 335297769    | TBD          | 📦     | 2026-02-02    |
| 2026-01-30 | RockAuto   | 42 parts (brake system, tune-up, cooling, clutch, + 3 tools)                   | 335576040    | TBD          | 🛒     | **\_**        |
| 2026-02-03 | Quadratec  | 3" Rough Country Lift Kit w/ Add-A-Leaf & N3 Shocks                            | 001623919    | 2026-02-08   | 📦     | 2026-02-08    |
| 2026-02-06 | Quadratec  | Brake lines, N3 stabilizer, shock isolators, D-ring bushings                   | 001625101    | 2026-02-12   | 📦     | 2026-02-12    |
| 2026-02-17 | RockAuto   | 11 items (brake booster, suspension, seals, hoses, clamps, exterior)           | 337239841    | TBD          | 🛒     | **\_**        |
| 2026-02-21 | RockAuto   | 5 items (control arms x2, parking brake cable, starter relay, CPS, MAP sensor) | 337591052    | TBD          | 🛒     | **\_**        |

---

## PARTS INSTALLATION LOG

Track when parts are installed:

| Date   | Part Installed | Labor (Hrs) | Notes  | Phase  |
| ------ | -------------- | ----------- | ------ | ------ |
| **\_** | **\_**         | **\_**      | **\_** | **\_** |
| **\_** | **\_**         | **\_**      | **\_** | **\_** |
| **\_** | **\_**         | **\_**      | **\_** | **\_** |

---

**Document Status:** Active Parts Tracker  
**Last Updated:** February 21, 2026  
**Next Review:** Update after each purchase and at end of each phase  
**Related Documents:** TOOLS_INVENTORY.md (for tool tracking)
